...
export default class DrawCalendar {
  ...
  drawAvaliableDate(date, item) {
    ...
    } else if (moment().isSameOrBefore(date, 'day')) {
      item.classList.add('available');
    } else {
      item.classList.add('unavailable');
    }
  ...
  selectDate(date) {
    ...
    const tdItem = this.tdArray.find((item) => item.dataset.date === pickedDate);
    if (tdItem) {
      tdItem.classList.add('active');
    }
  }
