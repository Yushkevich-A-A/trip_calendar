...
export default class WidgetController {
    ...
    addListenersClick() {
        document.addEventListener('click', event => {
            ...
            if (event.target.closest('.available')) {
                ...
                if (this.back.value !== '' && this.checkDate(this.there.value, this.back.value)) {
                    this.back.value = '';
    ...
    toggleCheckbox(element) {
        ...
        for(let i of [...document.querySelectorAll('.input-date')]) {
            i.value = '';
        }
    ...
    replaceDate(data) {
        let parseValue = data.split('-').join('')
        let value = parseValue.split('');
        if(!parseInt(parseValue)) {
            return '';
        }
    ...
    checkDate(there, back) {
        return moment(there, "DD-MM-YYYY").isAfter(moment(back, "DD-MM-YYYY"));
    }
    ...
    drawHandleDate() {
        ...
        if(this.currentInput.closest('.input-back')) {
            if (this.there.value === '' && !this.checkDateToCurrentTime()) {
                this.currentInput.value = moment().format("DD-MM-YYYY");
            } else if (!moment(this.there.value, "DD-MM-YYYY").isSameOrBefore(moment(this.currentInput.value, "DD-MM-YYYY"))) {
                this.currentInput.value = this.there.value;
            }

        } else if (!this.checkDateToCurrentTime()){
            this.currentInput.value = moment().format("DD-MM-YYYY");
        }
        this.calendar.drawCalendar(this.currentInput);
    ...
    checkDateToCurrentTime() {
        return moment().isSameOrBefore(moment(this.currentInput.value, "DD-MM-YYYY"));
    }
} 